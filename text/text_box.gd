...
@onready var next_indicator = $NinePatchRect/Control2/AnimatedSprite2D
...
func display_text(text_display: String, speach_sfx: AudioStream):
	text = text_display
	label.text = text_display
	audio_player.stream = speach_sfx
	await resized
	...
	global_position.y -= (size.y + 24)*0.8
	label.text = ""
